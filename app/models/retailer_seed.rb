class RetailerSeed < ActiveRecord::Base
  
  set_table_name 'retailers'
  ...
  validates_presence_of [ :city, :state ], :if => lambda{ |obj| (obj.address =~ RegPat) != 0 }, :message => "City and Country required if the Address was not a Lat / Lng coordinate pair"
  ...
  # NOTE: geocoding is handled in the form via a google maps api call so this is used to run seeds.rb
  ...
  before_validation :reset_coordinates,     # conditionally null out latitude / longitude so we can tell afterwards by their presence if the after_validation :geocode worked correctly
    :if => lambda{ |obj| obj.address_changed? || obj.city_changed? || obj.state_changed? }
  after_validation :geocode, :if => lambda{ |obj| obj.address_changed? || obj.city_changed? || obj.state_changed? }
    # :if => lambda{ |obj| obj.address_changed? || obj.city_changed? || obj.state_changed? } # || obj.latitude == nil || obj.longitude == nil}
